...
async fn main(req: Request, env: Env, _: Context) -> Result<Response> {
    ...
    Router::with_data(config)
        ...
        .on_async("/sub", sub)
        .on("/link", link)
        .on_async("/dns-query", dns::handle_dns_request)
...
fn link(_: Request, cx: RouteContext<Config>) -> Result<Response> {
    ...
    let vless_link = format!("vless://{uuid}@{host}:443?encryption=none&type=ws&host={host}&path=%2FKR&security=tls&sni={host}#kirlia");
    let trojan_link = format!("trojan://{uuid}@{host}:443?encryption=none&type=ws&host={host}&path=%2FKR&security=tls&sni={host}#kirlia");
    let ss_link = format!("ss://{}@{host}:443?plugin=v2ray-plugin%3Btls%3Bmux%3D0%3Bmode%3Dwebsocket%3Bpath%3D%2FKR%3Bhost%3D{host}#kirlia", URL_SAFE.encode(format!("none:{uuid}")));
